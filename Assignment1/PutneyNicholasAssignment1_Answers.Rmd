...
ipw_ate_u40 <- treatweight(y = y1_u40, # take initial data 
                        ...
                        boot = 100) # number of bootstrap replications
...
# Creating a data frame to summarize results
ipw_table_df <- data.frame(
  Group = c("Under 40", "Over 40"),
  Effect = c(ipw_ate_u40$effect, ipw_ate_o40$effect),
  SE = c(ipw_ate_u40$se, ipw_ate_o40$se),
  P_Value = c(ipw_ate_u40$pval, ipw_ate_o40$pval),
  Y1 = c(ipw_ate_u40$y1, ipw_ate_o40$y1),
  Y0 = c(ipw_ate_u40$y0, ipw_ate_o40$y0)
)

# Displaying results in a nice way
ipw_gt <- ipw_table_df %>%
  gt() %>% tab_header(title = "Inverse Probability Weighting (IPW) Results",
    subtitle = "Estimated ATE by Age Group") %>%
  fmt_number(columns = c(Effect, SE, Y1, Y0), decimals = 3) %>%
  fmt_number(columns = P_Value, decimals = 4) %>%cols_label(
    Group = "Age Group", Effect = "ATE", SE = "Std. Error",
    P_Value = "p-value", Y1 = "Mean (Treated)", Y0 = "Mean (Untreated)"c
  )
...
reg_monthly_ipw <- function(y, d, x) {
  ipw <- treatweight(y = y, d = d, x = x, ATET = FALSE, trim = 0, boot = 100)
