...
summary(ols.2b, vcov = sandwich, diagnostics = TRUE)
```
...
### (d)
In (b) only a single instrument (having two kids of the same sex) is used, and (c) uses two instruments (having two boys or having two kids) jointly. This allows for the effect of having two kids on having a third kid to be different depending on the sex of the kid. The estimated impact in (c) is greater than that of (b) (0.2260 vs. 0.1081) but still insignificant, indicating neither reveals an effect of having a third kid on hours worked by men. The estimates from the first stage of (b) and (c) indicate that there are similar percentage of compliers when using the two instruments (6% vs. 5-8% dependending on gender composition).
...
One would control for these variables if they influence both fertility and hours worked by the mothers. I do not think these variables are enough to alleviate potential endogeneity concerns. For example, things like religion and socioeconomic status may be correlated with having more kids and also have a causal relationship with the number of hours worked per week by mothers. If this were to be the case, the assumption of unfoundedness would be violated and our causal effect unidentifiable. 


### (c)
```{r}
#lm(morekids ~ samesex, data = data_um)
# "agem", "agefstm", "blackm", "hispm", "othracem"

tsls_2c <- ivreg(weeksm ~ morekids + agem + agefstm + blackm + hispm + othracem | samesex + agem + agefstm + blackm + hispm + othracem, data = data_um)
...
-> other answer
To estimate the effect of fertility in labor supply in Switzerland today, we'd likely have to change the definition of our exposure. This is due to the fact that many people are having less than two children. One way to do this is to look at the impact of having one kid, instead of having no kids. A potential instrumental variable for this exposure is fertility status, which takes one if the household is fertile and zero otherwise. Then, the first step would regress a dummy that indicates whether the household has at least one kid against the instrument. Then, the estimates of this exposure variable will be used in a model with labor supply outcomes as the response.
...
### (b)
The narrower bandwidth in women is due to a more non-linear relationship with the forcing variable compared to men. This leads to more variation around the cutoff. A smaller bandwidth improves the accuracy of the local linear approximation in the presence of such variation. 
